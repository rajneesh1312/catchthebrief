...
Session 6: Runs at 8:00 AM IST (2:30 AM UTC) daily.
...
def generate_brief(candidate, gemini, groq):
    # If a manual_brief is provided, skip AI entirely and use it as-is
...
def generate_article_page(brief, image_url, slug, article_index, total):
    ...
    replacements = {
        ...
        "{{SOURCE_NAME}}": brief["source_name"], "{{SOURCE_LINK}}": brief["source_link"],
        "{{WHATSAPP_URL}}": wa_url, "{{TWITTER_URL}}": tw_url,
        "{{IMAGE_URL}}": image_url or get_default_image(brief["category"]),
        "{{IMAGE_ALT}}": brief["title"],
        "{{ARTICLE_INDEX}}": str(article_index), "{{TOTAL_ARTICLES}}": str(total),
    }
    html = template
...
def save_archive(briefs_data, now):
    ...
    archive = {
        "date": iso_date(now), "generated_at": now.isoformat(),
        "briefs": [
            {"title": brief["title"], "category": brief["category"], "slug": slug,
             "source": brief["source_name"], "url": f"{SITE_URL}/articles/{slug}.html"}
...
{chr(10).join(day_blocks)}
  ...
  <footer>
    <div class="footer-inner">
      <div><div class="footer-logo">Catch<span>The</span>Brief</div><div class="footer-tagline">India's daily tech &amp; startup briefing</div></div>
...
def main():
    print("=" * 60)
    print("CatchTheBrief — Generate & Publish (Session 6, Step 2)")
    ...
    for i, candidate in enumerate(candidates):
        ...
        print(f"  Extracting og:image from {candidate['url'][:60]}...")
        image_url = extract_og_image(candidate["url"])
        if image_url:
            print(f"  Image found: {image_url[:60]}")
        else:
            print(f"  No og:image — using default")
    ...
    for i, (brief, image_url, slug) in enumerate(briefs_data):
        html = generate_article_page(brief, image_url, slug, i + 1, len(briefs_data))
        if html:
